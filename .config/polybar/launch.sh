...
for MONITOR in $(polybar -m | cut -d: -f1); do
  ...
  systemd-run --user --unit "$unitName" --property=Restart=always --collect --nice 19 --setenv MONITOR --slice "polybar-$location.slice" --property=TimeoutStopSec=2 -- polybar "$location" -c "$XDG_CONFIG_HOME/polybar/config-$location.ini"
done
